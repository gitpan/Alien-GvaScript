...
How many milliseconds to wait after a keypress before displaying suggestions.

=item typeAhead

If true, the current suggestion will be automatically inserted
...
Which color to put in the background when a "strict" field contains an illegal value
(default is red)
